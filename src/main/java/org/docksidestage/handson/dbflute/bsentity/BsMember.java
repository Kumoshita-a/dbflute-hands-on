...
public abstract class BsMember extends AbstractEntity implements DomainEntity {
     ...
     * 有効な会員住所(引数の日時時点で有効。現在日時を渡せば現在住所) <br>
     * Optional: alwaysPresent(), ifPresent().orElse(), get(), ...
     ...
     * 有効な会員住所(引数の日時時点で有効。現在日時を渡せば現在住所)
     ...
     * 最新のログイン(会員ごとに最大のLOGIN_DATETIME) <br>
     * Optional: alwaysPresent(), ifPresent().orElse(), get(), ...
     ...
     * 最新のログイン(会員ごとに最大のLOGIN_DATETIME)
